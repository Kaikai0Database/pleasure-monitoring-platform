"""
測試數據生成腳本 - 長期數據驗證
創建一個新的病人帳戶，並生成從 2025/11/1 到 2026/1/3 的評估記錄
...
def create_long_term_test_patient():
    """創建長期測試病人帳戶（2025/11/1 到 2026/1/3）"""
    app = create_app()
    ...
    with app.app_context():
        ...
        if existing_user:
            ...
            user = User(
                ...
                created_at=datetime(2025, 10, 25)  # 在11/1之前創建帳戶
            )
            db.session.add(user)
            db.session.commit()
        ...
        # 生成評估記錄
        start_date = datetime(2025, 11, 1)  # 2025年11月1日
        end_date = datetime(2026, 1, 3)     # 2026年1月3日
        times = [
            (9, 0),   # 早上9點
            (15, 0),  # 下午3點
            (21, 0)   # 晚上9點
        ...
        expected_records = expected_days * len(times)
        print(f"預計天數: {expected_days} 天")
        print(f"預計記錄數: {expected_records} 筆")
        print("-" * 50)
        ...
        while current_date <= end_date:
            for hour, minute in times:
                ...
                if record_count % 30 == 0:
                    print(f"  已生成 {record_count} 筆記錄... ({current_date.date()})")
        ...
        print(f"  每日測驗次數: {len(times)} 次")
        print(f"  總記錄數: {record_count} 筆")
        print(f"\n此帳戶用於驗證趨勢圖功能，包含64天的完整測試數據！")
        print(f"可以測試7日、14日、30日移動平均線的完整顯示效果。")

if __name__ == "__main__":
    create_long_term_test_patient()
