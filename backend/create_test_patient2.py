"""
測試數據生成腳本 (第二個病患)
創建一個新的病人帳戶，並生成從 2025/11/26 到 2025/12/26 的評估記錄
...
def create_recent_test_patient():
    """創建最近期間的測試病人帳戶"""
    app = create_app()
    ...
    with app.app_context():
        ...
        if existing_user:
            ...
            user = User(
                ...
                created_at=datetime(2025, 11, 20)  # 在11/26之前創建帳戶
            )
            db.session.add(user)
            db.session.commit()
        ...
        end_date = datetime(2025, 12, 26)  # 今天
        times = [
            (9, 0),   # 早上9點
            (15, 0),  # 下午3點
            (21, 0)   # 晚上9點
        ...
        print(f"\n此帳戶包含今天的數據，可用於測試當日圖表功能！")

if __name__ == "__main__":
    create_recent_test_patient()
